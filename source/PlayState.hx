...
class PlayState extends MusicBeatState
{
	...
	private var exfloatshit:Float = 0;
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'hater':
				dialogue = CoolUtil.coolTextFile(Paths.txt('hater/diag'));
		}
		...
		boyfriend = new Boyfriend(880, 490, SONG.player1);
		...
		healthBar.createFilledBar(0xFFFF3399, 0xFFFFFFFF);
		// healthBar
		add(healthBar);
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'hater' && curBeat >= 32 && curBeat < 160 && camZooming && FlxG.camera.zoom < 1.35)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}

		if (curSong.toLowerCase() == 'hater' && curBeat >= 192 && curBeat < 480 && camZooming && FlxG.camera.zoom < 1.35)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}

		if (curSong.toLowerCase() == 'from-hell' &&  curStep > 128 && curStep < 1408 && camZooming && FlxG.camera.zoom < 1.35)
		{
			floatshit += 0.02;
			exfloatshit += 0.01;
			FlxG.camera.zoom += 0.015;
		...
		if (curSong.toLowerCase() == 'from-hell' &&  curStep > 1664 && curStep < 1932 && camZooming && FlxG.camera.zoom < 1.35)
		{
			exfloatshit += 0.5;
		...
		if (curSong.toLowerCase() == 'from-hell' &&  curStep > 1983 && curStep < 3008 && camZooming && FlxG.camera.zoom < 1.35)
		{
			exfloatshit += 0.002;
